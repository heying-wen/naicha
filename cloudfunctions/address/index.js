...
async function updataDefaultAddress(id,openid){
  const where = {
    _id : _.neq(id),
    openid,
    isDefault:true,
...
exports.main = async (event, context) => {
  ...
  app.router('add',async(ctx)=>{
    ...
    if(!isObject(address) || isEmpty(address)){
      ...
      try{
        ...
        if(count >= MAX_ADDRESS_NUM){
          ctx.body={
            success: 0,
            message:'最多保存' + MAX_ADDRESS_NUM + '条地址'
          }
        }else{
      ...
      await updataDefaultAddress(curAddressId,ctx.openid)
      ctx.body = {
        success: 1,
  ...
  app.router('update',async(ctx)=>{
    ...
    if(!isObject(address) || isEmpty(address) || !id){
      ...
      //如果当前是默认地址，把其他地址修改为非默认
      await updataDefaultAddress(id,ctx.openid)
      ctx.body = {
        success: 1
      }
    }
